...
static int nano_send(int argc, char** argv) {
    ...
    if(E_SUCCESS != nl_address_to_public(dest_public_key, argv[2])){
        loading_disable();
        menu8g2_display_text_title(m, "Invalid Address", TITLE);
        ESP_LOGE(TAG, "\nInvalid Address %s\n", argv[2]);
        return_code = 1;
        goto exit;
    }
    ...
    if( 0 != mbedtls_mpi_read_string(&transaction_amount, 10, argv[3]) ) {
        loading_disable();
        menu8g2_display_text_title(m, "Invalid Amount", TITLE);
        ESP_LOGE(TAG, "\nInvalid Amount %s\n", argv[2]);
        return_code = 2;
        goto exit;
    }
    ...
    switch(nanoparse_web_process(new_block)){
        case E_SUCCESS:
            break;
        default:
            loading_disable();
...
static int nano_count(int argc, char** argv) {
    ...
    uint32_t block_count = nanoparse_web_block_count();
    printf("%d Blocks\n", block_count);
    return 0;
}
#if 0
...
void console_nano_register(subconsole_t *subconsole ) {
