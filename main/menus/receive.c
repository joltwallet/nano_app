...
typedef struct {
    ...
    struct {
        ...
        uint8_t open : 1;     /**< indicates if Receive Block needs to be an "open"
                                 state type */
    } block;
...
{
    if( jolt_gui_event.short_clicked == event ) { vault_refresh( NULL, step1, NULL ); }
}
...
static void step1( void *param )
{
     ...
     ***********************************************/
    if( !nano_get_private_public_address( NULL, d->block.receive.account, d->my_address ) ) { goto exit; }
...
static void step2( hex256_t pending_block_hash, mbedtls_mpi *amount, void *param, jolt_gui_obj_t *scr )
{
    ...
    sodium_hex2bin( d->block.receive.link, sizeof( uint256_t ), pending_block_hash, sizeof( hex256_t ), NULL, NULL,
                    NULL );
...
static void step3( nl_block_t *frontier_block, void *param, jolt_gui_obj_t *scr )
{
    ...
    if( NULL == frontier_block ) {
        ...
        d->block.open = true;
        nl_address_to_public( d->block.receive.representative, CONFIG_JOLT_NANO_DEFAULT_REPRESENTATIVE );
    }
    else {
    ...
    mbedtls_mpi_add_abs( &( d->block.receive.balance ), &d->transaction_amount, &d->block.frontier->balance );
...
static void step4_1( void *param )
{
    ...
    if( d->block.open ) { nano_network_work_bin( d->block.receive.account, step5, d, d->scr.progress ); }
    else {
...
static void step5( uint64_t work, void *param, jolt_gui_obj_t *scr )
{
    ...
    /* Not directly passing the progress screen because you cannot reliably cancel
