...
typedef struct {
    ...
    struct {
        nl_block_t *frontier; /**< Account frontier; an account cannot send funds
                                 unless it has a frontier */
...
static void entry_to_mpi( jolt_gui_obj_t *num_scr, mbedtls_mpi *val )
{
     ...
     *     1. Convert the user entered value into a string (no decimal, dealing
     * with raw)
    ...
    for( i = 0; i < CONFIG_JOLT_NANO_SEND_DIGITS; i++ ) { dest_amount_buf[i] = user_entry[i] + '0'; }

    /* Add enough zeros to represent the value in raw */
    for( uint8_t j = CONFIG_JOLT_NANO_SEND_DECIMALS; j < 30; i++, j++ ) { dest_amount_buf[i] = '0'; }
...
static void processing_cb_6( uint64_t work, void *param, jolt_gui_obj_t *scr )
{
    ...
    nano_network_process( &d->block.send, processing_cb_7, NULL,
                          NULL );  // Cannot reliably cancel at this stage.
    return;
}

static void processing_cb_7( esp_err_t status, void *param, jolt_gui_obj_t *scr )
{
    if( ESP_OK == status ) { jolt_gui_scr_text_create( TITLE_SEND, "Transaction Complete" ); }
    else {
...
static void contact_cb( jolt_gui_obj_t *btn_sel, jolt_gui_event_t event )
{
    if( jolt_gui_event.short_clicked == event ) {
        ...
        d->scr.entry = jolt_gui_scr_digit_entry_create( TITLE_SEND, CONFIG_JOLT_NANO_SEND_DIGITS,
                                                        CONFIG_JOLT_NANO_SEND_DECIMALS );
        if( NULL == d->scr.entry ) {
