...
#define JOLT_ERR_CHECK( x ) \
    if( E_SUCCESS != x ) { goto exit; }

#define CMD_PREAMBLE                                     \
    char *rpc_command = malloc( NANOPARSE_CMD_BUF_LEN ); \
    if( NULL == rpc_command ) { return ESP_FAIL; }       \
    cb_struct_t *s;                                      \
...
#define CALL_CB( ... )                              \
    ...
    if( NULL != cb ) { cb( __VA_ARGS__, p, scr ); } \
    return;
...
{
    CMD_PREAMBLE;
    snprintf( rpc_command, NANOPARSE_CMD_BUF_LEN, "{\"action\":\"block_count\"}" );
...
esp_err_t nano_network_work( const hex256_t hash, nano_network_work_cb_t cb, void *param, jolt_gui_obj_t *scr )
{
    ...
    strupr( hash_upper );
    snprintf( (char *)rpc_command, NANOPARSE_CMD_BUF_LEN, "{\"action\":\"work_generate\",\"hash\":\"%s\"}",
              hash_upper );
    CMD_POSTAMBLE( nano_network_work_cb );
}

esp_err_t nano_network_work_bin( const uint256_t hash_bin, nano_network_work_cb_t cb, void *param,
                                 jolt_gui_obj_t *scr )
{
    hex256_t buf_hex;
...
esp_err_t nano_network_frontier_hash( const char *account_address, nano_network_frontier_hash_cb_t cb, void *param,
                                      jolt_gui_obj_t *scr )
{
    CMD_PREAMBLE;
    snprintf( (char *)rpc_command, NANOPARSE_CMD_BUF_LEN, "{\"action\":\"accounts_frontiers\",\"accounts\":[\"%s\"]}",
              account_address );
...
{
    CMD_PREAMBLE;
    snprintf( (char *)rpc_command, NANOPARSE_CMD_BUF_LEN, "{\"action\":\"block\",\"hash\":\"%s\"}", block_hash );
                                       ...
                                       jolt_gui_obj_t *scr )
{
    ...
    cb_struct_t *s;
    s = malloc( sizeof( cb_struct_t ) );
    if( NULL == s ) { return ESP_FAIL; }
    s->cb    = cb;
    s->param = param;

    return nano_network_frontier_hash( address, nano_network_frontier_block_cb, s, scr );
}
